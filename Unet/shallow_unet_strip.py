...
class U_Net_In(tf.keras.Model):
    ...
    def __init__(self):
        ...
        # Conv2D(filters=n_filters, kernel_size=(3,3), padding='same', activation='relu', use_bias=True)

        # self.conv1 = Conv2D(filters=n_filters, kernel_size=(3, 3), padding='same', kernel_initializer='he_normal')
        self.conv1 = Conv2D(filters=n_filters, kernel_size=(3,3), padding='same', activation='relu', use_bias=True)
        self.p1 = MaxPool2D((2, 2))
        self.d1 = Dropout(0.1)

        # self.conv2 = Conv2D(filters=n_filters * 2, kernel_size=(3, 3), padding='same', kernel_initializer='he_normal')
        self.conv2 = Conv2D(filters=n_filters*2, kernel_size=(3,3), padding='same', activation='relu', use_bias=True)
        self.p2 = MaxPool2D((2, 2))
        self.d2 = Dropout(0.1)

        # self.conv3 = Conv2D(filters=n_filters * 4, kernel_size=(3, 3), padding='same', kernel_initializer='he_normal')
        self.conv3 = Conv2D(filters=n_filters*4, kernel_size=(3,3), padding='same', activation='relu', use_bias=True)
        ...
        self.mid_conv_in = Conv2D(filters=n_filters * 4, kernel_size = (3, 3), padding='same', activation='sigmoid', use_bias=True)
    ...
    def call(self, inputs):
        ...
        mid_conv_in = self.mid_conv_in(d3)
        return mid_conv_in
...
class U_Net_Out(tf.keras.Model):
    def __init__(self):
        ...
        # Conv2D(filters=n_filters * 4, kernel_size = (3, 3), padding='same', activation='sigmoid', use_bias=True)

        self.mid_conv_out = Conv2D(filters=n_filters * 4, kernel_size = (3, 3), padding='same', activation='sigmoid', use_bias=True)
        ...
        self.d4 = Dropout(0.1)
        self.conv4 = Conv2D(filters=n_filters * 4, kernel_size = (3, 3), padding='same', activation='relu', use_bias=True)

        self.upconv2 = UpSampling2D((2, 2))
        self.d5 = Dropout(0.1)
        self.conv5 = Conv2D(filters=n_filters * 2, kernel_size = (3, 3), padding='same', activation='relu', use_bias=True)

        self.upconv3 = UpSampling2D((2, 2))
        self.d6 = Dropout(0.1)
        self.conv6 = Conv2D(filters=n_filters * 4, kernel_size = (3, 3), padding='same', activation='relu', use_bias=True)
